...
def ingest_ticket(file_path):
    ...
    status = "closed" if "auto-closed" in tags else "new"

    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute(
        """
        INSERT INTO tickets (external_id, body, tags, status, enrichment_done)
        VALUES (%s, %s, %s, %s, FALSE)
        ON CONFLICT (external_id) DO NOTHING
        RETURNING id
    """, (external_id, body.strip(), tags, status))
    row = cur.fetchone()
    conn.commit()
    cur.close()
    conn.close()
    print(f"Ingested ticket: {external_id} | tags: {tags} | status: {status}")
